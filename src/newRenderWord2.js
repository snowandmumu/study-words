...
export default class RenderOneWord extends PureComponent {
    ...
    requireInfo = (w) => {
        ...
        // 加载图片1
        // import(`./wordlist/${word}/list/sense1/image/${word}.jpg`).then(res => {
        //     this.setState({
        //         sense1Img: res.default
        //     });
        // }).catch(err => {});
        ...
        // 加载图片2
        // import(`./wordlist/${word}/list/sense2/image/${word}.jpg`).then(res => {
        //     this.setState({
        //         sense2Img: res.default
        //     });
        // }).catch(err => {});
    ...
    componentWillUnmount() {
        ...
        // const currenWordsInfo = JSON.parse(localStorage.getItem(today));
        // const duration = new Date().getTime() - Number(localStorage.getItem('startTime'));
        // if (currenWordsInfo?.[word]) {
        //     currenWordsInfo[word] = Math.max(currenWordsInfo[word], duration);
        // }
        // else {
        //     currenWordsInfo[word] = duration;
        // }
        // console.log('willmount', currenWordsInfo);
        // localStorage.setItem(today, JSON.stringify(currenWordsInfo));
    }
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevProps.word !== this.props.word) {
            ...
            const duration = now - Number(localStorage.getItem('startTime'));
            console.log(now,  Number(localStorage.getItem('startTime')));
            if (currenWordsInfo?.[prevProps.word]) {
                currenWordsInfo[prevProps.word] = Math.max(currenWordsInfo[prevProps.word], duration);
            }
            else {
                currenWordsInfo[prevProps.word] = duration;
            }
            console.log('didupdate', currenWordsInfo);
            localStorage.setItem(today, JSON.stringify(currenWordsInfo));
            localStorage.setItem('startTime', new Date().getTime());
        }
    }
                            ...
                            if (senseItem) {
                                return (
                                    <div className={`sense sense${index + 1}`} key={word}>
                                        {/* {targetImg && <div className='sense-img'><img className='img' src={targetImg} /></div>} */}
                                        <div className='sense-info'>
                                            ...
                                            <div id="card-container" className='word-example-list'>
                                                {/* {senseItem?.wordDefPhrases.map((phrase, phraseIndex) => {
                                                    const phraseAudio = `${hostName}/audios/${word}_sense_${index + 1}_def_phrase_${phraseIndex}.mp3`;
                                                        return (
                                                            <div
                                                                id={phraseIndex.toString()}
                                                                className={`word-example-item ${currentAudio === phraseAudio ? 'current' : ''}`}
                                                                key={`${word}_example_${phraseIndex}`}
                                                            >
                                                                <div className='word-example-item-left'>
                                                                    <img className='img' src={`${hostName}/images/${word}_sense_${index + 1}_def_phrase_${phraseIndex}.png`} />
                                                                </div>
                                                                <div className='word-example-item-right'>
                                                                    <span className='dic-example-audio'>
                                                                        <AudioPlayer url={phraseAudio} />
                                                                    </span>
                                                                    {phrase}
                                                                </div>
                                                            </div>
